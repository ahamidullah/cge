...
{
	return light_ambient * vec3(texture(material.diffuse, t_tex_coords));
}

vec3 calc_diffuse(vec3 light_diffuse, vec3 recv_dir)
{
	float diffuse_impact = max(dot(t_normal, recv_dir), 0.0f);
	return light_diffuse * diffuse_impact * vec3(texture(material.diffuse, t_tex_coords));
}
...
vec3 calc_specular(vec3 light_specular, vec3 recv_dir, vec3 view_dir)
{
	...
	return light_specular * specular_impact * vec3(texture(material.specular, t_tex_coords));
}
...
void main()
{
	...
	// directional lighting
	{
		...
		amb_result = calc_ambient(dir_light.ambient) + calc_diffuse(dir_light.diffuse, amb_recv_dir) + calc_specular(dir_light.specular, amb_recv_dir, view_dir);
	}
	// point lights
	{
		vec3 pt_recv_dir;
